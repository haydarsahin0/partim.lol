/**
 * create-club-subscription — kendi futbol kulübünü kurmak isteyen kullanıcı
 * için haftalık Stripe aboneliği başlatır.
 *
 * Kulüp, ödeme onaylanmadan açılmaz: gerçek kayıt stripe-webhook fonksiyonunda,
 * apply_football_club_subscription() ile yapılır. Burada yalnızca doğrulama ve
 * Checkout oturumu var. Ücret parti kurmayla aynıdır (haftalık $19).
 *
...
Deno.serve(async (req) => {
  ...
  if (name.length < 3 || name.length > 40) return json({ error: "Kulüp adı 3–40 karakter olmalı." }, 400);
  ...
  // Renk, mevcut kulüplerin hiçbirine yakın olmamalı — istemciye güvenmiyoruz.
  const { data: clubs, error: clubsError } = await supabase
    .from("football_clubs")
    .select("name,color,full_name");
  if (clubsError) return json({ error: clubsError.message }, 500);

  for (const club of clubs ?? []) {
    if (colorDistance(color, club.color) < MIN_COLOR_DISTANCE) {
      return json({ error: `Renk ${club.name} rengine çok yakın.` }, 409);
    }
    if ((club.full_name ?? "").toLowerCase() === name.toLowerCase()) {
      return json({ error: "Bu adda bir kulüp zaten var." }, 409);
    }
  }
      ...
      line_items: [
        {
          ...
          price_data: {
            ...
            product_data: {
              ...
              description: "Haftalık kulüp aboneliği. Oyun içi dijital konum; gerçek kulüple bağlantısı yoktur.",
            },
          },
        },
      ],
