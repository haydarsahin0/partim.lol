-- Zaman tüneli verisi sayfalanarak okunuyor; sıralama KESİN olmalı.
--
-- NEDEN
--
-- İstemci artık oy geçmişini sayfa sayfa çekiyor: tek istekte dönen satır
-- sayısı sınırlanabildiği için yanıt sessizce kırpılıyordu ve videodaki il
-- oranları gerçeğiyle tutmuyordu.
--
-- Ama sayfalama ancak sıralama TAM ise doğru çalışır. Fonksiyon yalnızca
-- kovaya göre sıralıyordu; aynı kovadaki satırların kendi arasındaki sırası
-- belirsizdi. Belirsiz sırada sayfalamak, sayfa sınırındaki satırların bir
-- kısmını iki kez, bir kısmını hiç getirmez — yani kırpılmayı düzeltirken
-- yeni bir bozulma açardı.
--
-- Sıralama artık (kova, il, parti) üçlüsü üzerinde: her satırın yeri tek.
