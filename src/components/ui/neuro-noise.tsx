...
/**
 * "Neuro Noise" — partim.lol arka plan sahnesi.
 *
 * Paper Shaders'tan uyarlandı (https://shaders.paper.design/neuro-noise),
 * Apache-2.0. Kütüphanesiz düz WebGL1: tam ekran tek üçgen çizilir, tüm iş
 * fragment shader'da yapılır. Sekme arka plandayken ve `prefers-reduced-motion`
 * açıkken döngü durur; devicePixelRatio 2 ile sınırlanır.
 */
...
/** Reçetedeki ilk beş renk (low → high) */
...
function compile(gl: WebGLRenderingContext, type: number, source: string) {
  ...
  if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
    console.error("[neuro-noise] shader derlenemedi:", gl.getShaderInfoLog(shader));
...
export function NeuroNoise({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    // WebGL yoksa sayfanın kendi arka planı devreye girer.
    if (!gl) return;
    ...
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
      console.error("[neuro-noise] program bağlanamadı:", gl.getProgramInfoLog(program));
      return;
    }
    gl.useProgram(program);

    // Tam ekranı tek üçgenle kapla: quad'a göre bir vertex az ve dikiş yok.
    ...
    gl.uniform4f(uSurface, 1.82, 1.0, -0.03, 1.48);
    gl.uniform4f(uFinish, 0.09, 0.0, 0.001, 0.1);
    ...
    const render = (now: number) => {
      resize();
      const seconds = ((reduceMotion ? start : now) - start) / 1000;
      gl.uniform4f(uScene, width, height, seconds * 0.86, 5.0);
      gl.uniform4f(uSpace, 0.0, 0.0, pointer.x, pointer.y);
      gl.uniform4f(uCursor, pointer.presence, 3.0, 0.45, 0.46);
